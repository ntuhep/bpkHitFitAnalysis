...
void jetMetSystematics::scale()
{
   ...
   if( JES == _type || JER == _type ) { //JES or JER
      ...
      for( int j = 0; j < _jets->Size; j++ ) {
         ...
         _jets->PtCorrL2[j]  *= ptscale;
         _jets->PtCorrL3[j]    *= ptscale;
         _jets->PtCorrL7g[j] *= ptscale;
         _jets->PtCorrL7uds[j] *= ptscale;
         _jets->PtCorrL7c[j] *= ptscale;
         _jets->PtCorrL7b[j]   *= ptscale;
         _jets->Px[j] *= ptscale;
         _jets->Py[j] *= ptscale;
         _jets->Pz[j] *= ptscale;
         _jets->Energy[j] *= ptscale;
