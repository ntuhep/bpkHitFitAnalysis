...
bpkHitFitAnalysis::prioritizeBtags( std::vector<int> jets )
{
   if( debug ) {
      ...
      for( int j = 0; j < ( int )jets.size(); j++ ) {
         double tagvalue = 0;
         if( bTagAlgo.compare( "CSV" ) == 0 )  { tagvalue = JetInfo.pfCombinedSecondaryVertexV2BJetTags[jets[j]]; }
         else if( bTagAlgo.compare( "TCHE" ) == 0 ) { tagvalue = JetInfo.pfTrackCountingHighEffBJetTags[jets[j]]; }
...
bpkHitFitAnalysis::jetIsBTagged( int ijet )
{
   ...
   if( bTagAlgo.compare( "CSV" ) == 0 )  { return ( JetInfo.pfCombinedSecondaryVertexV2BJetTags[ijet] > bTagCut ); }
   if( bTagAlgo.compare( "TCHE" ) == 0 ) { return ( JetInfo.pfTrackCountingHighEffBJetTags[ijet] > bTagCut ); }
